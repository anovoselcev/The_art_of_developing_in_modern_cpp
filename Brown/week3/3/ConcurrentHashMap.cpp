...
public:
  ...
  explicit ConcurrentMap(size_t bucket_count) : _umap(bucket_count), _mut(bucket_count){}

  WriteAccess operator[](const K& key){
	  auto index = hasher(key)%_umap.size();
	  auto& um = _umap[index];
	  auto& m = _mut[index];
	  return{lock_guard<mutex>(m),um[key]};
  }
  ReadAccess At(const K& key) const{
	  ...
	  if(auto it = um.find(key); it!=um.end())
		  return{lock_guard<mutex>(m),it->second};
	  throw out_of_range("");
  }
  ...
  bool Has(const K& key) const{
	  auto index = hasher(key)%_umap.size();
	  auto& um = _umap[index];
	  lock_guard<mutex> guard(_mut[index]);
	  return um.find(key) != um.end();
  }

  MapType BuildOrdinaryMap() const{
	  MapType res;
	  size_t size = _umap.size();
	  for(size_t i = 0;i<size;i++){
		  lock_guard<mutex> guard(_mut[i]);
		  for(const auto& x : _umap.at(i))
			  res.insert(x);
	  }
	  return res;
  }
...
private:
  Hash hasher;
  vector<MapType> _umap;
  mutable vector<mutex> _mut;
};
