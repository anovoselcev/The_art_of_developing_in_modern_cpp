...
public:
  ...
  friend ostream& operator << (ostream& output, const HttpResponse& resp){
	  output<<"HTTP/1.1"<<' '<<static_cast<size_t>(resp._code);
	  if(resp._code == HttpCode::Ok)
		  output<<' '<<"OK";
	  else if(resp._code == HttpCode::NotFound)
		  output<<' '<<"Not found";
	  else
		  output<<' '<<"Found";
	  output<<'\n';
	  ...
	  if (req.method == "POST") {
		  if (req.path == "/add_user") {
			  ...
			  if (banned_users.count(user_id) == 0) {
				  ...
				  h.SetCode(HttpCode::Found);
				  h.AddHeader("Location","/captcha");
			  }
		  }
